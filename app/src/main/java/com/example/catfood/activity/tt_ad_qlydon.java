    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        lv_donhang.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                // Lấy đối tượng donhang được chọn
                donhang selectedDonHang = donhangad.get(position);

                // Lấy danh sách chi tiết đơn hàng
                ArrayList<ChiTietDonHang> chiTietList = databasecuatuan.hienthiChiTietDonHang(selectedDonHang.getMdon());

                // Khởi chạy Activity hoặc Dialog để hiển thị chi tiết đơn hàng
                Intent intent = new Intent(tt_ad_qlydon.this, tt_ad_qlydonchitiet.class);
                intent.putExtra("selectedDonHang", selectedDonHang);
                intent.putExtra("chiTietList", chiTietList);
                startActivity(intent);
